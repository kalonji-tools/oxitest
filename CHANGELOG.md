...
- Tear down the worker's FixtureSession after each task (#1728)
- Assert Cargo.lock satisfies Cargo.toml, not that deps are newest (#1730)
- Drain diagnostics after scope teardown, not only after tests (#1709)
- Report module-lifetime fixtures in the cache stats (#1709, #1734)
- Register __fixtures__.py fixtures in parallel workers (#1732)
- Await and dispose async @oxi.fixture declarations (#1733)
- Forward __str__ and __format__ through FrozenProxy (#1735)
- Keep a package-lifetime subtree inside one dispatch phase (#1710)
- Address the post-implementation review (#1713)
- Correct the documented lifetime of session fixtures (#1769)
- Decide doctest entry staleness statically, not from run outcomes (#1796)
- Base doctest staleness on files actually parsed, not files offered (#1796)
- B1-filter autouse candidate enumeration (#1774)
- Cache function-lifetime fixtures once per test (#1775)
- Fold per-test cache counters at dispose instead of discarding them (#1775)
- Repair 7 stale rustdoc intra-doc links and gate preflight on cargo doc (#1807)
- Report files that fail to parse in doctest coverage (#1800)
- Lint test targets in both clippy gates (#1815)
- Correct the shared-fixture parallel warning (#1778)
- Report fixture shadowing from visibility rather than name equality (#1766)
- Keep inline fixtures registered on warm-cache runs (#1850)
- Widen the warm-cache signal to aliased fixture imports (#1850)
- Serialize collection errors into the CTRF report (#1682)
- Write the --json artifact on every early exit (#1682)
- Stream worker diagnostics to the LDJSON pipe (#1840)
- Surface worker diagnostics through the reporter (#1840)
- Read a worker's output tail before reaping it (#1840)
- Fail closed on the async fixture guard parameter (#1876)
- Raise when TestContext identity is read outside a test (#1874)
- Build the worker's fixture session once per process (#1777)
- Drain process-lifetime fixtures once per coordinator run (#1777)
- Two assertions matched a path, not the diagnostic (#1777)
- Stop the keep-tmp test recursing through the whole system temp dir (#1777)
- Restore the direnv hook's blocking guarantee (#1891)
- Import declaration homes when prescan cannot name the spelling (#1859)
- Source ADR-0009 package and Rule 4 checks from the registry (#1859)
- Enforce the inline lifetime cap in the registrar (#1859)
- Key declaration lookup on the defining file, not the anchor (#1859)
- Prescan recognises declarations carrying extra keywords (#1716)
- Sandbox coverage integration tests to their own tmp dir (#1648)
- Write the --junit-xml artifact on every early exit (#1858)
- Activate plugins in worker processes (#1717)
- Register plugin providers once per worker, and cover what was missed (#1717)
- Key package disposal by the declaring anchor (#1839)
- Dispose nested declaring packages at their ancestor's boundary (#1839)
- File async teardowns under their declared boundary (#1839)
- Drain nested async boundaries innermost-first on the worker path (#1839)
- Exit non-zero when the worker payload cannot be serialized (#1832)
- Derive the rootdir package from the declared test tree (#1798)
- Fold only declared testpaths that hold test files (#1798)
- Audit doctest coverage over the declared tree, not the walked one (#1798)
- Audit from the rootdir when the project declares no testpaths (#1798)
- Report scope and skip entries outside the declared test tree (#1798)
- Register ancestor declaration homes up to the rootdir package (#1765)
- Bound the rootdir-package fold to the project (#1921)
- Type skip() as NoReturn and document runtime skipping (#1947)
- Report a callback registered from inside a teardown (#1952)
- Resolve temp directories at creation so paths compare equal (#1957)
- Fail the test that leaves a worker's working directory deleted (#1957)
- Bind a fixture's dependencies to its own teardown boundary (#1958)
- Give doctest modules a stable, cache-reused identity (#1962)
- Report and complete teardowns that raise (#1966)
- Register fixture teardowns before advancing the generator (#1962)
- Remove the two private version constants from the stability page (#1785)
- Windows verbatim path prefix made every node ID a glob (#1989)
- Oxitest inspect hangs instead of exiting when stdout is redirected (#1989)
- Internal traceback frames were never filtered on Windows (#1989)
- Durations were measured with a 15.6ms-granularity clock on Windows (#1989)
- A timed-out test was counted but never named (#1989)
- The encoding gate could not see a partial-wrapped spawner (#1989)
- Async tests no longer double-arm their timeout on Windows (#1998)
- A Windows timeout can no longer fire into an unrelated later test (#1998)
- A worker can read a task carrying a non-ASCII path (#2004)
- FdCapture round-trips non-ASCII output under any locale (#2004)
- A non-protocol line on worker stdout no longer deletes a result (#2010)
- A worker survives undecodable bytes on its own stdout (#2010)
- The collection route reuses an already-imported source module (#2014)
- Quote every justfile interpolation so a value survives the shell (#2015)
- Compare file identity in already_imported (#2018)
- Refuse a Target that does not exist (#1797)

### Features


- @oxi.fixture declaration layer — registry, registrar, dispatch (#1708)
- Prescan and collect same-package __fixtures__.py (#1708)
- Wire fixture-module registration through the Rust/Python bridge (#1708)
- Add lifetime="module" fixture tier (#1709)
- Add .envrc and auto-approve it on worktree creation (#1737)
- Worker tasks carry N modules, wire protocol v5 (#1745)
- Package lifetime instantiates exactly once per run (#1710)
- Add lifetime="session" + lifetime-cap enforcement (#1711)
- Inline @oxi.fixture in test_*.py, capped at module (#1712, #1757)
- Enforce the ADR-0009 B1 boundary (#1713)
- Shortcut access fx.<name>, unconditionally legal (#1714)
- Append the project rootdir to sys.path (#1780)
- Thread rootdir into the serial and inspect session paths (#1780)
- Carry rootdir over the worker wire, protocol v6 (#1780)
- [**breaking**] Remove the query helpers resource (#1788)
- [**breaking**] Remove the inspect Helper node kind (#1788)
- [**breaking**] Remove the HelperProvider plugin protocol (#1788)
- [**breaking**] Unwire the helper registry from session and conftest loading (#1788)
- [**breaking**] Delete Helpers, the helpers proxy and HelperRegistry (#1788)
- Move async session cleanup to the process boundary (#1777)
- Rename lifetime="session" to lifetime="process" (#1777)
- Warn when a killed worker skips process-lifetime teardowns (#1777)
- Declare autouse fixtures with @oxi.fixture(autouse=True) (#1716)
- Order autouse firing and report suppressed autouse (#1716)
- Add PluginModuleSource and dispatch it everywhere (#1717)
- Resolve plugin fixture homes and register their declarations (#1717)
- Prescan activated plugin packages for fixtures (#1717)
- Add ambient access to the running test's context (#1949)
- FdCapture returns the descriptor bytes it cannot decode (#2009)
- [**breaking**] Remove fx.oxi.ctx (#2022)

### Performance


- Memoise path decomposition in the visibility predicates (#1853)
