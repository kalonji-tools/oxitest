...
| a dependency or release version | the query and its output, **and the question it answered** — "what is newest" and "does version X exist" are different queries, and `.claude/skills/verify-finding/SKILL.md` names the command for each | **the date it was queried** | a version recalled rather than queried, **including one recalled from model training**; a newest-version output offered as evidence about a different version |
...
The version row names **model training** as a source because that failure has a direction the others do not, and because it fails *upward*. A remembered measurement is as likely to be stale high as stale low, so it reads as uncertain. A remembered version is confidently specific about a release that may not exist: **#1577 is titled `bump pyo3 from 0.29 to 0.31.x`, closed `COMPLETED`, and `pyo3 0.31` has never been published.** `Cargo.toml:13` still pins `"0.29"`, and the registry's version list holds no 0.30 and no 0.31. `cargo search pyo3 --limit 1` returns `pyo3 = "0.29.2"` — the newest version, which is not evidence about 0.31 either way, and answering that took the existence query. An issue was written, triaged and closed against a version number that came from nowhere but recall.

The date is required because the answer expires fast: `gh release list` shows **seven releases in the 27 days to 2026-08-09** (`v2.0.0` … `v4.0.0`), so a recalled version is wrong within weeks and the recall carries no signal that it has gone stale. Which command answers which question is the skill's table rather than this one — this row states what evidence a version claim needs, and every *version* command lives in that one file so the two cannot disagree. They did: this row and that table prescribed opposite `git tag` forms for one morning (#2151).
