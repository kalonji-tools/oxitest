...
**A gate over these copies is refused, on measurement (#2124).** A search for `protocol v[0-9]` returns 9 lines, of which 6 are correct — two of them in `CHANGELOG.md`, which git-cliff writes and nobody may edit by hand. A search cannot separate *"the version is 7"* from *"this changed at v3"*, and the numeric literals need a second pattern that sees none of the first set. Delete the copy instead: a literal that does not exist cannot go stale. The defect had already recurred once (#1589, v2 against v3) before six copies were removed across three files.
...
5. **Test file names say what the file pins, not which issue prompted it.** The issue number belongs in the commit title, which stage 9 already requires — a name carrying it is a second copy of a fact another consumer owns. `test_returns_none.py`, never `test_2067_returns_none.py`. Four files drifted from this in eight days before it was written down (#2080), so it is prose that has already been broken once; a fifth occurrence is the evidence for making it a gate.
