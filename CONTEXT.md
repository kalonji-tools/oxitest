...
**Wire Protocol** — The LDJSON protocol between workers and the Rust coordinator. Each stdout line has a `"type"` discriminator: `"result"` (test outcome), `"diagnostic"` (user-facing), or `"trace"` (developer). Missing `"type"` defaults to `"result"`, which is backwards compatibility with a worker older than the version that introduced the discriminator. **This glossary holds no version number for it.** `PROTOCOL_VERSION` in `src/worker_result/wire.rs` declares the version, `python/oxitest/_bridge/result.py` mirrors it, and `scripts/check_bridge_sync.py` refuses a wire-shape change that leaves the two out of step. A number written here would be a third copy that no gate reads — which is what this entry and the **Worker** entry below each became, disagreeing with the constant and with each other (#2109).
