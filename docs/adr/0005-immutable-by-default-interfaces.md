...
| `Helpers` | Accumulates helper definitions via `@helpers.helper` decorator during conftest loading |
...
- **FrozenProxy limitation.** `FrozenProxy` intercepts `__setattr__`, `__delattr__`, `__setitem__`, and `__delitem__`, but cannot intercept arbitrary mutating method calls (e.g., `.append()` on a wrapped list) at runtime. `ty` static analysis is the primary enforcement layer for this case.
