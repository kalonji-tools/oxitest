...
**The guarantee is scoped to directory-anchored fixtures.** Declaration homes (`__fixtures__.py`, `__init__.py`) are registered run-wide — on the serial path and into every worker — so the full view is genuinely full and the verdict is deterministic. An inline declaration is registered only when its test module is imported, so its presence in the catalog depends on worker assignment, selection, and import order. Rather than let the *diagnostic* vary with scheduling, cross-module access to an inline fixture reported `FixtureNotFoundError` with a static hint that inline declarations are capped at `module` lifetime. [#1758](https://github.com/kalonji-tools/oxitest/issues/1758) narrowed that: a literal `fx.<stem>.<name>` is read out by prescan and refused at collection with `BoundaryError`, identically in serial, at `-n 1` and at `-n 2`. Only a dynamic `getattr` still reaches the access-time gate and still varies. Semantics are unaffected in every reading — the access is illegal in all of them. The message no longer advises the qualified form for a test module stem ([#1759](https://github.com/kalonji-tools/oxitest/issues/1759)); the remaining dynamic-access difference is deferred there as non-goal 2.
...
- **The guarantee is scoped to directory-anchored fixtures.** Inline declarations are registered on module import, so their presence in the "full" catalog varies with worker assignment and import order. Rule 5 scoped the `BoundaryError` guarantee to declaration homes rather than let a diagnostic depend on scheduling. [#1758](https://github.com/kalonji-tools/oxitest/issues/1758) later moved the static route ahead of that problem: a literal `fx.<stem>.<name>` is refused at collection time, so only a dynamic `getattr` is still scoped out.
...
### Amendment 17 — Consequence #4's fallback exists, on a variant it never named (2026-08-11)

Tracked by [#1727](https://github.com/kalonji-tools/oxitest/issues/1727). Amends the *Fallback to Python-import discovery* consequence. Amendment 4's audit block is **not** amended: it recorded what was true on 2026-07-30 and is kept as the audit record, exactly as the note above it says.

**The fallback was built by [#1859](https://github.com/kalonji-tools/oxitest/issues/1859), not by this ticket.** Commit `092f5e49`, *"import declaration homes when prescan cannot name the spelling"*, merged 2026-08-04. The consequence's intent — a declaration prescan cannot name falls through to the runtime — is live. Only the trigger was wrong, and it was wrong in a way that made it unreachable: `prescan_fixture_module` returns `Unavailable` on a read or parse failure and on nothing else, so the dynamic-decoration snippet the bullet used to illustrate itself never took that path.

Measured on Linux 6.18.41 x86\_64, `oxitest 4.0.0`, each run after `rm -rf .oxitest_cache`:

| `__fixtures__.py` | Prescan verdict | Outcome |
|---|---|---|
| `if flag: dec = oxi.fixture(...)` then `@dec` — the bullet's own illustration | `NoFixtures(has_decorated_functions)` | fixture resolves, `1 passed`, exit 0 |
| `import oxitest as ox` then `@ox.fixture(...)` | `NoFixtures(has_decorated_functions)` | fixture resolves, `1 passed`, exit 0 |
| a fixture name no file declares (control) | — | `fixture 'nonexistent' not found`, exit 3 |

**What shipped is sounder than what was promised.** The flag is not a claim about oxitest: the decorator is never inspected. Registration is by marker attribute, so `import oxitest as ox` declares a real fixture that no static scan can see, and any trigger keyed on decorator *shape* is defeated by a spelling. Asking the runtime is the only answer that cannot be, which is why the mechanism is recorded here rather than treated as an implementation detail.

**Considered Option 2's rejection stands.** It refused a runtime import hook partly because that "breaks under the standard three-tier collection fallback when AST prescan fails". That argument leaned on the fallback existing, and it does.
