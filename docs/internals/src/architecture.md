...
- **Fixture lifecycle** (`_bridge/_fixture_session.py`, `_bridge/_fixture_registry.py`) -- scope-based caching, yield teardown, autouse firing.
...
| `inspect::detail` | `src/inspect/detail/mod.rs` | Per-type detail renderers split across submodules: `fixture.rs`, `test.rs`, `mark.rs`, `declaration.rs`, `plugin.rs`, `styles.rs`. |
...
| `inspect::graph` | `src/inspect/graph/mod.rs` | `InspectGraph`, `NodeRef`, `NodeKind`. |
| `inspect::graph::nodes` | `src/inspect/graph/nodes.rs` | 5 node structs: `FixtureNode`, `TestNode`, `MarkNode`, `DeclarationNode`, `PluginNode`. |
