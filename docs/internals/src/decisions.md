...
| Pipeline typestate | v0.12 | [#22](https://github.com/kalonji-tools/oxitest/issues/22) | `Pipeline<S>` uses phantom-type states (`Empty`, `FilesCollected`, `Prescanned`, ..., `Executed`) so that phase ordering is enforced at compile time. Calling `.collect()` on a `Pipeline<Empty>` is a type error -- the compiler rejects it before any test runs. This eliminates an entire class of "forgot to call X before Y" bugs. See the [Pipeline Deep Dive](pipeline.md) chapter for the full state diagram. |
...
| Pipeline deepening | post-v0.14 | [PR #857](https://github.com/kalonji-tools/oxitest/pull/857), [PR #858](https://github.com/kalonji-tools/oxitest/pull/858) | `Pipeline<S>` now holds `PipelineShared` directly (with `Deref`/`DerefMut` access) instead of going through `SetupContext`. The monolithic `phases.rs` file splits into `phases/` with one file per pipeline state. `filter_metadata()` is rewritten with `file_matches_*` predicate functions in `filter.rs`. `execute()` uses `PoolGuard` RAII and extracted `report_violations()` / `emit_shared_fixture_warning()` helpers. |
