...
| `[tool.oxitest.doctest]` | sub-table | — | Doctest collection + coverage. Presence of the table enables the rule (default `scope = "public"`); absence disables it. `scope` accepts either the scalar `"public"` or a list of node-ID-style entries (`"path/to/dir/"`, `"path/to/mod.py"`, `"path/to/mod.py::sym"`, `"path/to/mod.py::Cls::method"`). `skip` uses the same list grammar and subtracts from the resolved subject set. An entry naming a path that does not exist is stale on every invocation; a `Symbol`/`Member` entry is additionally stale when its file was scanned but the symbol was not found. Stale entries surface via the global `strict` dial. See [Use doctests](../how-to/use-doctests.md). CLI `--doctest-modules` maps to `scope = "public"`. |
