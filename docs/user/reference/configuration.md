...
| `[tool.oxitest.doctest]` | sub-table | — | Doctest collection + coverage. Presence of the table enables the rule (default `scope = "public"`); absence disables it. `scope` accepts either the scalar `"public"` or a list of node-ID-style entries (`"path/to/dir/"`, `"path/to/mod.py"`, `"path/to/mod.py::sym"`, `"path/to/mod.py::Cls::method"`). `skip` uses the same list grammar and subtracts from the resolved subject set. Stale entries (matching zero subjects) surface via the global `strict` dial. See [Use doctests](../how-to/use-doctests.md). CLI `--doctest-modules` maps to `scope = "public"`. |
