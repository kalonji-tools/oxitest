...
def _record(event: str) -> None:
    with Path(f"{os.environ['SLICE4_LOG']}.{os.getpid()}").open(
        "a", encoding="utf-8"
    ) as fh:
        fh.write(f"{event}\n")
...
def _use(fx: Fixtures) -> None:
    ...
    _record(f"USE gamma {os.getpid()} {engine}")


def test_gamma_one(fx: Fixtures) -> None:
    _use(fx)


def test_gamma_two(fx: Fixtures) -> None:
    _use(fx)
