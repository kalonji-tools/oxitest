...
def _record(event: str) -> None:
    with Path(os.environ["SLICE9_PROC_LOG"]).open("a", encoding="utf-8") as fh:
        fh.write(f"{event}\n")


def test_delta_one() -> None:
    _record(f"USE delta {os.getpid()} unrequested")


def test_delta_two() -> None:
    _record(f"USE delta {os.getpid()} unrequested")
