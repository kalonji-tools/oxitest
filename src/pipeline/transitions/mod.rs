...
//!
//! Those guards are exception **E1**, the only entry on
//! `docs/adr/0011-no-unhandled-panic-routes.md`'s list. The exception is
//! typestate-only: a site here that guards on anything but the phase gets
//! fixed, not absorbed.
#![allow(
    clippy::unreachable,
